...
use context::{gl43_core as gl, opengl, render_target::RenderTarget, Context};
...
fn winit_main() -> anyhow::Result<(), anyhow::Error> {
    env_logger::Builder::from_env(env_logger::Env::default().default_filter_or("warn"))
        .format_timestamp(None)
        .init();
    ...
    //let mut context = opengl::Context::new(GLContext(gl_context))?;
