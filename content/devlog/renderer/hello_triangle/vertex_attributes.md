...
VAO's basically explain which vertex attribute<sup>1</sup> uses data
from which buffer and how the data needs to be interpreted. In short: It makes
...
"Vertex Atrribute 0 is in Buffer 1, begins at position BYTE_OFFSET, it has 3 components(x, y, z) of the type F32, and the offset between each vertex is STRIDE_BYTES".
...
normal0, position1, color1, normal1, ..]`, which also known as interleaving, group them
per attribute `[position0, position1, .., color0, color1, .., normal0, normal1, ..]`, or have
the attributes in any other order/position in one or more buffer objects. Though,
it is not possible to take a single attribute(like the positions) and splitting
...
That said, there is no direct relationship between models/meshes and VAOs. The
VAO just stores the sources of our vertex streams and their layouts.
We could have all vertices from all our models/meshes inside a single buffer and use the same VAO for all of them.
In that case we just need to pass the our draw function the index of the first
vertex of the mesh we want to draw, in addition to the number of vertices that
belong to that mesh, but more about that later.

There can only one active VAO at a time.
...
> <sup>1</sup> Vertex Attributes are per-Vertex data. Things like the position, color, texture coordinate and normals of a vertex
...
```
Creating the actual object almost exactly as creating the buffer objects, plural
...
And again, we are technically required to delete the objects manually with
...
There are 3 different functions, that converts our data into different types. 
...
scalars(single value) or vectors<sup>1</sup>. We will just
look at `glVertexAttribPointer` for now, that is the float variant of these
functions.

> <sup>1</sup> To clarify, these have nothing to do with the Vec data structure.    
...
the positions, which are interleaved, start at 0, and each position is made out
...
Generally when working with new OpenGL functions, I skim through the Errors section at
...
at the bottom of the doc, which leads us to the different get functions.
...
`glVertexAttribPointer` will always convert integers to floats!

The normalized parameter is only relevant if our vertex data is made out of said
integers. If it's false, it will convert them to via a c-style cast, taking the
values as they are, "putting a .0 behind them".
...
all values of our attribute. Since we only have our position for now, we are
...
of the byte size of the attribute).

Finally, we come to the "pointer" attribute, which is an awkward pointer because
this function is being reused. It was used to pass the pointer to the data
before vertex buffer were invented, but in "modern" OpenGL it's just a relative byte offset to
...
//gl::VertexAttribPointer(0, 3, gl::FLOAT, gl::FALSE, 0, (0 as *const usize).cast());  alternative with cast
```
...
later. Both of these functions are state associated with our VAO.
...
again. It will no forever and ever store that "Attribute 0 is in MY_BUFFER".  
...
we will tell OpenGL how many vertices we want to draw.
...
OpenGL 4.3, introduced a new set of functions, that splits the data source from
...
Potatohead" at the start of the buffer, followed b y lots of nonsense, then the
...
Where the Potatohead and the Speedwagon start is done later.
...
Important to notice is that `bindingindex` has nothing to do with out attribute
index. Instead, we will bind our buffer to this index.

Then we just specify our buffer(no more need to bind it anymore!).
The offset to the first element in the buffer for out layout. 
...
Finally, there is the stride again, keeping it's meaning. It tells how many
...
our format doesn't care how it's stored.
...
don't provide the attributes.

So for us it means: 
```rust
gl::BindVertexBuffer(0, vbo, 0, std::mem::size_of::<f32>() as i32 * 3);  //the distance to the next vertex is always size of a single position(3 * size of f32).
```
...
```rust 
gl::VertexAttribBinding(0, 0); //attribute 0 uses buffer at bindingindex 0
```
